...
func TestRedEnvelopeService_Receive(t *testing.T) {
	accountService := aservices.GetAccountService()
	convey.Convey("收红包测试用例", t, func() {
		accounts := make([]*aservices.AccountDTO, 0)
		size := 10
		// 1. 准备多个账户，用于发红包和收红包
		for i := 0; i < size; i++ {
			account := aservices.AccountCreateDTO{
				UserId:       ksuid.New().Next().String(),
				Username:     "测试用户" + strconv.Itoa(i+1),
				AccountName:  "测试用户" + strconv.Itoa(i+1),
				AccountType:  int(aservices.EnvelopeAccountType),
				CurrencyCode: "CNY",
				Amount:       "2000",
			}
